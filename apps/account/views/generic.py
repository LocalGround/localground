...
from localground.apps.account.models import UserProfile
...
def change_user_profile(request, template_name='user_prefs.html'):
    ...
    from localground.apps.account.forms import CustomUserChangeForm, UserProfileForm #, MapGroupForm
    page_num = 1
...
def send_invitation(request):
    ...
    """
    import re
    from localground.apps.helpers.emailer import Email
    ...
    #instantiate emailer:
    emailer = Email()
    mail_success = emailer.send_invitation(emails, request.user, subject=subject,
                                           body=body)
    if not mail_success:
        message = 'There was an error sending the invitation email'
    return HttpResponse(json.dumps({'success': mail_success, 'message': message}))
...
def get_my_contacts_autocomplete(request):
    ...
    if q is not None:
        emails = list(request.user.userprofile.contacts.filter(email__icontains=q)
                  .values_list('email', flat=True).order_by('email',)[:limit])
    else:
        emails = list(request.user.userprofile.contacts.values_list('email', flat=True)
...
def get_contacts_autocomplete(request, format='text'):
    ...
    if format != 'json':
        return HttpResponse('\n'.join(u.get('label') for u in user_recs), mimetype='text/plain')
    else:
