...
class ImageQuality():
    ...
    def init_image_statistics(self):
        ...
        self.brightest_cluster_average = int(
            scipy.mean(self.brightest_cluster))

        self.brightest_cluster_range = max(
            self.brightest_cluster)-min(self.brightest_cluster)
        self.dif_brightest_pixel_brightest_cluster = (abs(
            scipy.sum(self.brightest_pixel) -
            scipy.sum(self.brightest_cluster))
        )/3

        if sum(self.brightest_cluster)/3 >= 250:
            self.image_quality = ImageQuality.HIGH_QUALITY_SCAN
        elif sum(self.brightest_cluster)/3 >= 225 and \
                diff_brpixel_brcluster <= 10 and \
                self.brightest_cluster_range <= 10:
            self.image_quality = ImageQuality.MEDIUM_QUALITY_SCAN
        elif diff_brpixel_brcluster <= 20 and \
                self.brightest_cluster_range <= 10:
