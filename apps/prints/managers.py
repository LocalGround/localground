...
class PrintMixin(GeneralMixin):
    ...
    def my_prints(self, user, include_shared=False, with_scans=False,
        ...
        from localground.apps.prints.models import PrintPermissions
        ...
        if user is not None:
            ...
            if not include_shared: #just mine:
                prints = prints.filter(source_print__owner=user)
            if can_manage:
                prints = prints.filter(can_manage=True)
            if can_edit: 
                prints = prints.filter(can_edit=True)
            if with_scans:
                prints = prints.filter(num_scans__gt=0)
            prints = prints.order_by('-source_print__time_stamp',)
            ...
            prints = prints.order_by('-time_stamp',)
        return prints
    ...
    def my_prints_old(self, user, include_shared=False, include_public=False,
        ...
        if user is not None:
            if include_shared == include_public == True:
                prints = prints.filter(Q(owner=user) | Q(view_authority__id=3) |
                                       Q(printuser__auth_user=user))
            elif include_public:
                prints = prints.filter(Q(owner=user) | Q(view_authority__id=3))
            elif include_shared:
                prints = prints.filter(Q(owner=user) | Q(printuser__auth_user=user))
            else:
                prints = prints.filter(owner=user)
        else:
            prints = prints.filter(view_authority__id=3)  
...
class PrintManagerOld(models.GeoManager):
    def my_prints(self, user, include_shared=False, with_scans=False,
        ...
        if user is not None:
            ...
            if not include_shared: #just mine:
                prints = prints.filter(source_print__user=user)
            if can_manage:
                prints = prints.filter(can_manage=True)
            if can_edit: 
                prints = prints.filter(can_edit=True)
    ...
    def my_prints_old(self, user, include_shared=False, include_public=False,
        ...
        if user is not None:
            if include_shared == include_public == True:
                prints = prints.filter(Q(user=user) | Q(view_authority__id=3) |
                                       Q(printuser__auth_user=user))
            elif include_public:
                prints = prints.filter(Q(user=user) | Q(view_authority__id=3))
            elif include_shared:
                prints = prints.filter(Q(user=user) | Q(printuser__auth_user=user))
            else:
                prints = prints.filter(user=user)
        else:
            prints = prints.filter(view_authority__id=3)  
