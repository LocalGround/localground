...
from localground.apps.uploads.models import Scan, Attachment, Audio, Photo, Video, StatusCode
...
def export_file(format, ext, username, form, project=None, driver=None,
    ...
    extras = {
        'snippet_path':
            '\'http://\' || uploads_snippet.host || ' +
            'uploads_snippet.virtual_path || ' +
            'uploads_snippet.file_name_orig',
        'form_path':
            'select \'http://\' || a.host || \'/static/attachments/\' || a.uuid || \'/\' || ' +
            'a.file_name_orig from uploads_attachment a ' +
            'where a.id = uploads_snippet.source_attachment_id',
        'map_path':
