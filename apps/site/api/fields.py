...
class GeometryField(serializers.WritableField):
	...
	def __init__(self, *args, **kwargs):
		...
		if kwargs.get('definitely_required'):
			self.definitely_required = kwargs.pop('definitely_required') #this is a hack because the required keyword doesn't seem to be working!
	...
	def field_from_native(self, data, files, field_name, into):
		...
		if geom is None:
			if self.definitely_required:
				return None
			#ensure that the field is actually nullable...
			#if self.required:
			...
			if 'Point' in self.geom_types:
				...
				#GEOSGeometry(json.dumps({"type": "Point", "coordinates": [-123.31158757209778, 40.346797604717196]}))
			...
			for child in entities:
				...
				for id in ids:
					...
					try:
						...
						raise serializers.ValidationError(
							'No %s object exists with id=%s' % (overlay_type, id) #entity['id'])
						)
