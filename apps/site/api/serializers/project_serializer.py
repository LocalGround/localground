...
class ProjectDetailSerializer(BaseSerializer):
    ...
    def get_scans(self, obj):
        data = ScanSerializer(
                models.Scan.objects.get_objects(obj.owner, project=obj, processed_only=True), many=True
            ).data
        return self.serialize_list(models.Scan, data)

    def get_markers(self, obj, forms):
        data = MarkerSerializerCounts(
                models.Marker.objects.get_objects_with_counts(obj.owner, project=obj, forms=forms), many=True
            ).data
