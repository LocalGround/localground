...
class TileSetSerializer(AuditSerializerMixin, serializers.ModelSerializer):
    ...
    base_tile_url = serializers.CharField(
        source='tile_url', required=False, allow_null=True,
        label='base_tile_url',
        style={'base_template': 'textarea.html', 'rows': 5}, allow_blank=True
    )
    base_static_url = serializers.CharField(
        source='static_url', required=False, allow_null=True,
        label='base_static_url',
        style={'base_template': 'textarea.html', 'rows': 5}, allow_blank=True
    )
    ...
    class Meta:
        ...
        fields = (
            'id', 'url', 'name', 'tags', 'overlay_source', 'base_tile_url',
            'base_static_url', 'min_zoom', 'source_name', 'max_zoom',
            'is_printable', 'extras', 'key', 'owner', 'attribution')
        depth = 0
