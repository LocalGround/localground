...
echo "Create PostGIS Extension" | tee -a "$log_file"
	sudo -u postgres bash -c "psql -c \"CREATE EXTENSION postgis;\" -d $DB_NAME"
	sudo -u postgres bash -c "psql -c \"CREATE EXTENSION postgis_topology;\" -d $DB_NAME"
	sudo -u postgres bash -c "psql -d template1 -c \"create extension hstore;\""
	sudo -u postgres bash -c "psql -c \"CREATE EXTENSION hstore;\" -d $DB_NAME"
