...
define([
    ...
    function (Backbone, MainApp, BaseMapView, LeftPanelView, DataDetailView,
        ...
        const initApp = (scope) => {
            ...
            spyOn(MainApp.prototype, 'updateDisplay').and.callThrough();
        ...
        describe("MainApp vent listeners: ", function () {
            ...
            it("Listens for 'show-data-detail'", function() {
            ...
            it("Listens for 'show-modal'", function() {
            ...
            it("Listens for 'hide-modal'", function() {
        ...
        describe("MainApp functions: ", function () {
            ...
            it("getCenter() should return correct coordinates", function() {
                // to do...
                // these ones are more like integration tests...
            });

            it("updateDisplay() should add correct css", function() {
                mainApp.showLeft = false;
                mainApp.showRight = false;
                mainApp.updateDisplay();
                expect(mainApp.container.$el).toHaveClass('none');

                mainApp.showLeft = true;
                mainApp.showRight = false;
                mainApp.updateDisplay();
                expect(mainApp.container.$el).toHaveClass('left');

                mainApp.showLeft = false;
                mainApp.showRight = true;
                mainApp.updateDisplay();
                expect(mainApp.container.$el).toHaveClass('right');

                mainApp.showLeft = true;
                mainApp.showRight = true;
                mainApp.updateDisplay();
                expect(mainApp.container.$el).toHaveClass('both');
            });

            it("hideList() should set showLeft and call updateDisplay()", function() {
                expect(mainApp.updateDisplay).toHaveBeenCalledTimes(0);
                mainApp.hideList();
                expect(mainApp.showLeft).toEqual(false);
                expect(mainApp.updateDisplay).toHaveBeenCalledTimes(1);
            });
            it("unhideList() should set showLeft and call updateDisplay()", function() {
                expect(mainApp.updateDisplay).toHaveBeenCalledTimes(0);
                mainApp.unhideList();
                expect(mainApp.showLeft).toEqual(true);
                expect(mainApp.updateDisplay).toHaveBeenCalledTimes(1);
            });
            it("hideDetail() should set showLeft and call updateDisplay()", function() {
                expect(mainApp.updateDisplay).toHaveBeenCalledTimes(0);
                mainApp.hideDetail();
                expect(mainApp.showRight).toEqual(false);
                expect(mainApp.updateDisplay).toHaveBeenCalledTimes(1);
            });
            it("unhideDetail() should set showLeft and call updateDisplay()", function() {
                expect(mainApp.updateDisplay).toHaveBeenCalledTimes(0);
                mainApp.unhideDetail();
                expect(mainApp.showRight).toEqual(true);
                expect(mainApp.updateDisplay).toHaveBeenCalledTimes(1);
            });
        });
